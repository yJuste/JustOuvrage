	...
	var body: some View {
		GeometryReader { geo in
			ZStack {
				...
				FlameShape.shared
					.fill(
						LinearGradient(
							colors: [
								colors.first?.opacity(0.40) ?? .black,
								colors.last?.opacity(0.20) ?? .black,
								.clear
							],
							startPoint: .bottom,
							endPoint: .top
					...
					.offset(flame1Offset)
					.scaleEffect(
						x: animate ? 1.015 : 0.985,
						y: animate ? 1.03 : 0.97
					)
					.animation(
						active ? .easeInOut(duration: 22).repeatForever(autoreverses: true) : nil,
						value: animate
					)
					.blendMode(.screen)
				...
				FlameShape.shared
					.fill(
						LinearGradient(
							colors: [
								colors.last?.opacity(0.26) ?? .black,
								.clear
							],
							startPoint: .bottom,
							endPoint: .top
					...
					.animation(
						active ? .easeInOut(duration: 28).repeatForever(autoreverses: true) : nil,
						value: animate
					)
					.blendMode(.screen)
				...
				FlameShape.shared
					.fill(
						LinearGradient(
							colors: [
								colors.first?.opacity(0.45) ?? .black,
								colors.last?.opacity(0.20) ?? .black,
								.clear
							],
							startPoint: .bottom,
					...
					// 🔥 PLUS HAUT + PLUS COLLÉ GAUCHE
					.offset(
						x: -geo.size.width * 0.22,  // plus à gauche
						y: -geo.size.height * 0.40   // beaucoup plus haut
					)
					...
					.animation(
						active
						? .easeInOut(duration: 24).repeatForever(autoreverses: true)
						: nil,
						value: animate
					)
					.blendMode(.screen)
			...
			.onAppear {
				...
				flame1Start = randomStart(in: geo.size)
				flame1End = randomEnd(from: flame1Start, in: geo.size)
				
				flame2Start = randomStart(in: geo.size)
				flame2End = randomEnd(from: flame2Start, in: geo.size)
				
				// 🔥 FLAME 3 setup (haut gauche)
				flame3Start = CGSize(
				...
				flame3End = CGSize(
					width: flame3Start.width + 30,
					height: flame3Start.height + 120
				)
...
// MARK: - Shape optimisée (singleton → moins d’allocations)
