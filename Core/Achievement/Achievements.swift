...
enum Achievements: Identifiable, CaseIterable {
	...
	func pourcentage(in context: AchievementContext) -> Double {
		...
		case .deafened: return min(audioCount / 100.0, 1.0)
		case .silentNoMore: return min(audioCount / 250.0, 1.0)
		case .hearMeOut: return min(audioCount / 1_500.0, 1.0)
		case .inTheLoop: return min(audioCount / 3_000.0, 1.0)
		case .tunedIn: return min(audioCount / 5_000.0, 1.0)
		case .lostInTheMusic: return min(audioCount / 7_000.0, 1.0)
		case .soundMaster: return min(audioCount / 10_000.0, 1.0)
		case .audiophile: return min(audioCount / 20_000.0, 1.0)
